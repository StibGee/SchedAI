...
if (depid==0):
    cursor.execute("""
        ...
        LEFT JOIN (
            SELECT subjectname, subjecttype, departmentid, COUNT(*) AS specialization_count
            FROM facultysubject
            GROUP BY subjectname, subjecttype, departmentid
        ) AS fs ON fs.subjectname = subject.commonname 
                AND fs.subjecttype = subject.type 
                AND fs.departmentid = department.id
        WHERE subject.focus != 'Minor' AND subject.focus != 'Major1'
        AND subjectschedule.calendarid = %s
        AND department.collegeid = %s
        ORDER BY 
            subject.departmentid ASC,
            CASE WHEN subject.focus = 'OJT' THEN 0 ELSE 1 END ASC, -- Force 'OJT' rows to the end
...
def facultysubjectmatch(subjectschedulesubjectname, facultysubjectfsubjectname, subjectschedulesubjectmasters, facultysubjectmasters, subjectscheduledepartmentid, facultysubjectdepartmentid,facultysubjectype, subjectscheduletype):
    ...
    departmentmatch= (subjectscheduledepartmentid==facultysubjectdepartmentid)
    subjecttype = (facultysubjectype==subjectscheduletype)
    return subject_name_match and master_match and departmentmatch and subjecttype
...
unassigned_subjects=[]
unassigned_subjects = []  
facultyworkinghours = {faculties[0]: faculties[12] + faculties[21] for faculties in faculty}
assignedsubjects = {}
noassignment=[]
tbh=[]
backtrackcounters={}
...
def assign_subject(currentshubjectid):
    ...
    if (depid==0):
        cursor.execute("""
            ...
            JOIN faculty ON faculty.id = facultysubject.facultyid 
            WHERE faculty.collegeid = %s 
            ORDER BY faculty.departmentid ASC, faculty.type DESC, teachinghours DESC 
            """, (collegeid,))
    ...
    for faculties in faculty1:
        ...
        teaching_hours = faculties[12]+faculties[21]
    ...
    if (backtrackcounters[currentshubjectid] >= maxdepth):
        ...
        for facultysubjects in sortedfaculty1:
            ...
            facultysubjectmasters = facultysubjects[13]
            facultysubjectdepartmentid = facultysubjects[4]
            facultysubjectfname = facultysubjects[6]
            facultysubjectlname = facultysubjects[8]
        ...
        """
        values = ("NEW", 'FACULTY', 'Contractual', 30, 'Yes',collegeid,subjectscheduledepartmentid)
        cursor.execute(query, values)
        conn.commit()
        ...
        sortedfaculty = sorted(facultysubject1, key=lambda x: (x[15] != subjectscheduledepartmentid))
        
        for facultysubjects in sortedfaculty:
            facultysubjectfacultyid = facultysubjects[1]
            facultysubjectfsubjectname = facultysubjects[2]
            facultysubjectmasters = facultysubjects[13]
            facultysubjectype = facultysubjects[3]
            facultysubjectdepartmentid = facultysubjects[4]
            facultysubjectfname = facultysubjects[6]
            facultysubjectlname = facultysubjects[8]
            ...
            if facultysubjectmatch(subjectschedulesubjectname, facultysubjectfsubjectname, subjectschedulesubjectmasters, facultysubjectmasters, subjectscheduledepartmentid, facultysubjectdepartmentid, facultysubjectype, subjectscheduletype):
                ...
                if facultyworkinghourscheck(facultyworkinghours[facultysubjectfacultyid], subjectschedulesubjecthours, facultysubjectfacultyid):
                    ...
                    conn.commit()
                    progress = (assignedsubjectscount) / subjectschedulecount[0] * 100
                    print(f"{progress-1:.2f}%: {facultysubjectfsubjectname} {subjectscheduletype} {subjectschedulesection} assigned to {facultysubjectfname} {facultysubjectlname} {facultysubjectfacultyid}")
    ...
    print(f"failed assigning {currentshubjectid}/{len(subjectschedule)} {subjectschedulesubjectname} {subjectscheduletype}   {subjectscheduledepartmentid} , trying previous assignment.")
        ...
        for days in dayslist:
            ...
            for time in range(timestart, timeend, 30):
                if time not in sectionoccupied[departmentid][yearlvl][section][daysfinal]:
                    ...
                    sectionsubject.setdefault(departmentid, {}).setdefault(yearlvl, {}).setdefault(section, {}).setdefault(daysfinal, {})[time] = subsubid
...
'''def findfacultylastassignedup(facultyid, day):
    ...
    day_slots = facultyoccupied[facultyid].get(day, {})

    last_occupied = 0


    for time in range(420, 1441, 30): 
...
def findfacultylastassigneddown(facultyid, day, timeslot):
    if facultyid not in facultyoccupied or day not in facultyoccupied.get(facultyid, {}):
        return 0
...
def findsectionlastassignedup(departmentid, yearlvl, section, day, timeslot):

    department_data = sectionoccupied.get(departmentid, {})
    year_data = department_data.get(yearlvl, {})
    section_data = year_data.get(section, {})
    day_slots = section_data.get(day, {})
...
def assigningtimeslot(day, starttime, duration, hour, roomid, facultyid, departmentid, yearlvl, section, subjectid, subjectfocus):
    
    if subjectfocus=='OJT':
        subjectidsection=000
    else:
        subjectidsection=subjectid
    ...
    '''Section Assign'''
    for sectiontimeslot in range(starttime, starttime+duration, 30):
        sectionsubject.setdefault(departmentid, {}).setdefault(yearlvl, {}).setdefault(section, {}).setdefault(day, {})[sectiontimeslot] = subjectidsection
...
def backtrackingtimeslot(day, starttime, duration, hour, roomid, facultyid, departmentid, yearlvl, section, subjectid):
    ...
    '''Section Assign'''
    for sectiontimeslot in range(starttime, starttime+duration, 30):
        sectionsubject.setdefault(departmentid, {}).setdefault(yearlvl, {}).setdefault(section, {}).setdefault(day, {})[sectiontimeslot] = 'none'
...
def sectionsubjectcounter(departmentid, yearlvl, section, day, timeslot):
    ...
    for time in range(timeslot-30, 420 - 30, -30): 
        subject = sectionsubject[departmentid][yearlvl][section][day].get(time)
...
def sectionsubjectcounterdown(departmentid, yearlvl, section, day, timeslot):
    ...
    for time in range(timeslot, 1140 + 30, 30): 
        subject = sectionsubject[departmentid][yearlvl][section][day].get(time)
...
def facultysubjectcounterdown(facultyid, day, timeslot):
    facultysubject.setdefault(facultyid, {}).setdefault(day, {})
...
def resetstate():
    global sectionoccupied, sectionsubject, roomoccupied, facultysubject
    global facultyoccupied, backtrackcounters, facultyassignmentcounter
    global assignments, assignedsubjects, assignedsubjectscount, dayslist
    ...
    for rm in sorted_rooms:
        ...
        if units == 3.0 or (newroomlablol[currentsubjectid] and not newroomlabtried[currentsubjectid]):
            ...
            if(backtrackcounters[currentsubjectid] < maxdepth):
                ...
                for facultyidpair, slots in facultypairdaystime.items():
                    ...
                    for day1, day2, starttime, endtime in slots:
                        ...
                        if (sectionsubjectcounter(departmentid, yearlvl, section, day1, startminutes)==2 or sectionsubjectcounter(departmentid, yearlvl, section, day1, startminutes)==2):
                            continue

                        if (sectionsubjectcounter(departmentid, yearlvl, section, day2, startminutes)==2 or sectionsubjectcounter(departmentid, yearlvl, section, day2, startminutes)==2):
                            continue
                        ...
                        if ((findsectionlastassignedup(departmentid, yearlvl, section, day2,startminutes)!=0) and (startminutes-findsectionlastassignedup(departmentid, yearlvl, section,day2, startminutes)>210)):
                            continue
                        ...
                        if day1free and day2free and facultyday1free and facultyday2free:
                            assigningtimeslot(day1, startminutes, 90, 1.5, roomid, facultyidpair, departmentid, yearlvl, section, subjectid, subjectfocus)
                            assigningtimeslot(day2, startminutes, 90, 1.5, roomid, facultyidpair, departmentid, yearlvl, section, subjectid, subjectfocus)
            ...
            if (backtrackcounters[currentsubjectid] < maxdepth):
                ...
                for rm in sorted_room22:
                    ...
                    for daylabbacktrack in preferreddays: 
                        ...
                        if getfacultytype(subjectfacultyid)=='Regular':
                            if facultyhours is not None and facultyhours+2>6:
                                continue
                        ...
                        for timebacktrack in range(420, 1140, 30):
                            ...
                            if day1free and facultyday1free:         
                                assigningtimeslot(dayvalid, starttime, 120, 2, roomid, subjectfacultyid, departmentid, yearlvl, section, subjectid, subjectfocus)
                                lec2found=True
                        ...
                        for daybacktracklec2 in range(1,7): 
                            ...
                            for time in range(420, 1140, 30):
                                ...
                                if day1true and facultyday1true: 
                                    assigningtimeslot(daybacktracklec2, time, 120, 2, roomid, subjectfacultyid, departmentid, yearlvl, section, subjectid, subjectfocus)
                                    lec2found2=True
                ...
                lec2found22222=False
                if not lec2found22222: 
                    ...
                    for day1 in preferreddays: 
                        ...
                        for time in range(420, 1140, 30):
                            ...
                            if day1true and facultyday1true: 
                                ...
                                assigningtimeslot(day1, time, 180, 3, roomid, subjectfacultyid, departmentid, yearlvl, section, subjectid, subjectfocus)
                ...
                for rm in sorted_rooms1:
                    ...
                    for day1 in preferreddays: 
                        ...
                        for time in range(420, 1140, 30):
                            ...
                            if facultysubjectcounter(subjectfacultyid, day1, time)==2:
                                day1true = False
                                continue
                ...
                if not labfound:
                    ...
                    if not labfound2:
                        ...
                        check_query = """
                            SELECT COUNT(*) 
                            FROM `room`
                            WHERE `name` = %s AND `type` = %s AND `collegeid` = %s
                        """
                        values = ("FIELD", 'Lab', collegeid)
...
if depid == 0:
    ...
    DELETE FROM faculty
    WHERE fname = 'NEW'
    AND faculty.collegeid = %s
    AND id NOT IN (SELECT ss.facultyid FROM subjectschedule ss WHERE ss.facultyid IS NOT NULL);
    """
    param = (collegeid,)
else:
    query = """
